...
export default function MuiFnQTable(props: Props) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axios.post(`${BASE_API}/get_q_finished`, { craneId: props.craneId })
                if (res.status === 200) {
    ...
    const cols: GridColDef[] = [
        ...
        },
        {
            ...
            renderCell: (params: GridVal) => {
                const reportDetail = () => {
                    router.push(`/reports/${params.row.input_id}`)
                }
                const handleMouseDown = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
                    if (e.button === 1) {
                        ...
                        window.open(`/reports/${params.row.input_id}`, '_blank');
                    }
                };
                ...
                return (
                    <Tooltip title={'อ่านรายงาน'} placement="top">
                        <Button
                            ...
                            disabled={String(params.row.status) === '3'}
                            size="small"
                            variant="outlined">
                            <FindInPageIcon />
                        </Button>
